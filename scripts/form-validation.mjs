...
const sourcePath = join(rootDir, 'app', 'utils', 'form-validation.ts');
...
for (const projectName of projectNames) {
  ...
  const targetPath = join(appDir, 'utils', 'form-validation.ts');
...
if (copiedProjects.length === 0) {
  ...
  console.log(`Synced form-validation.ts to ${copiedProjects.join(', ')}.`);
}
